...
MLRegressionBoosting <- function (dataset = NULL, options, perform = "run", callback = function(...) 0, ...) {
	...
	# variablesToRead <- c(predictors,target)
	variablesToRead <- c(unlist(options$target), unlist(options$indicator), unlist(options$predictors))
	...
	if(is.null(dataset) && !is.null(target) && !is.null(predictors)){
		...
		if (perform == "run"){
			...
			dataset <- .readDataSetToEnd(columns = variablesToRead, exclude.na.listwise = NULL)
			# dataset <- .readDataSetToEnd(columns = variablesToRead, exclude.na.listwise = variablesToRead)
	...
	if(perform == "run" && !is.null(dataset)){
		...
		customChecks <- list(
			...
			function(){
				if(options[["testBox"]]=="manual_percentageTrain" && (options[["value_percentageTraining"]] < 0.1)){
			...
			function(){
				if(options[["OOBBox"]]=="manual_subsample" ){
					...
					if (options[["value_subsample"]] > 0.95){
						return("Percentage of Subsample Too Large!")
					}
				}
			},
			...
			function(){
				if(options[["TreeStructureBox"]] == "optimized_TS"){
					...
					if(options[["value_MinTermNodeSideMin"]] > options[["value_MinTermNodeSideMax"]]){
						return("Not a a valid range for n.minobsinnode")
					}
				}
			}
		)
		...
		if(!hasError){
			...
			res <- .BoostingAnalysis(dataset, options, predictors, target, indicator, perform = perform)
	...
	results[["tableFE"]] <- .tableFE(res,target, predictors,perform)
	if(hasError){
		results[["tableFE"]][["error"]] <- errorList
	}
	...
	if (options[["tableChisq"]])
		results[["tableChisqTest"]] <- .tableChisqTest(res, target, predictors, perform)
	...
	if(options[["TreeStructureBox"]] == "optimized_TS")
		results[["tableBestTune"]] <- .tableBestTune(res$optStruc, options, predictors, target, perform = perform)

	if(!is.null(indicator))
		results[["tablePrediction"]] <- .tablePrediction(res$appData, res$prediction, predictors, target, perform = perform)
...
.BoostingAnalysis <- function(data, options, predictors, target, indicator, perform = perform){
	...
	if(is.null(indicator)){
		...
		# print(typeof(data[,indicatorDotV]))
		# print(data[,indicatorDotV])
		# if(!is.logical(data[,indicatorDotV])){
		# 	if(is.factor(data[,indicatorDotV]))
		# 		appIdx <- as.logical(as.numeric(as.character(data[,indicatorDotV])))
		# 	if(is.character(data[,indicatorDotV]))
		# 		appIdx <- as.logical(as.numeric(data[,indicatorDotV]))
		# }else{
		# 	appIdx <- data[,indicatorDotV]
		# }
	...
	if (class(model) == "try-error") {
		errorMessage <- .extractErrorMessage(model)
	}
	...
	return(list(model=model, appData = appData, prediction = prediction, data = fitData, best.iter = best.iter, best.iter_cv = best.iter_cv, best.iter_test = best.iter_test, best.iter_oob = best.iter_oob, optStruc=optStruc))
...
.tableChisqTest <- function(res, target, predictors, perform){
...
.tableBestTune <- function(data, options, predictors, target, perform = perform){
...
.tablePrediction <- function(appData, prediction, predictors, target, perform = perform){

	print("in .tablePrediction")
	table <- list(title="Model Prediction")
	colNames <- c(unlist(predictors), sprintf("Predicted %s", target))
	mFeatures <- length(predictors)
	if(perform == "init"){

		toTable <- matrix(".", nrow = 1, ncol = mFeatures + 1,
				  dimnames = list(".", colNames))
...
.plotHstatistics <- function(HValueAll, colVarIdx, rowVarIdx){
	...
	text(1:length(HValueAll),0.3, labels=sprintf("%0.2f",HValueAll),cex=2)
	# if (colVarIdx==1){
